...
class E_Moderator {
  ...
  /**
   * @var int $mod_id The ID of the moderator
   */
  public $mod_id;
  ...
  public static function get_instance( $mod_id ) {
    ...
    $_moderator = self::query("SELECT * FROM moderators WHERE mod_id = $mod_id LIMIT 1");
   ...
   * @return void
   *
   * @var int $mod_id The primary key of the moderator being registered, as created in moderators database
   *
   * @todo Test
   */
   ...
   * @return void
   *
   * @var int $mod_id The primary key of the moderator being registered, as created in moderators table
   *
   * @todo Test
   */
  ...
  private static function email_exists( $mod_email ) {
    global $edb;
    $moderators = $edb->select('moderators', 'mod_id,mod_email', "mod_email = '$mod_email'");
    if (!empty($moderators))
      return true;
    else
      return false;
  ...
  private static function login_name_exists( $mod_login_name ) {
    global $edb;
    $moderators = $edb->select('moderators', 'mod_id,mod_login_name', "mod_login_name = '$mod_login_name'");
    if (!empty($moderators))
      return true;
    else
      return false;
  ...
  public static function authenticate_moderator( $mod_login_name, $mod_pass ) {
    global $edb;
    $moderators = self::query("SELECT * FROM moderators WHERE mod_login_name = '$mod_login_name' AND mod_pass = '$mod_pass' ORDER BY mod_id DESC LIMIT 1");
    foreach ( $moderators as $moderator ) {
        get_class($moderator);
        foreach ( $moderator as $key => $value )
          $key = $value;
...
function login_moderator( $login_name, $password ) {
  ...
  if ($mod_id > 0) {
    ...
    echo '<META HTTP-EQUIV="Refresh" Content="0; URL=profile.php?profile='.$mod_id.'">';
    exit;
  }
  else {
